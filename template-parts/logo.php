...
<a class="logo btn-menu-toggle" href="#" aria-controls="primary-menu" aria-expanded="false">
    <?php if (has_custom_logo()): ?>
        <img class="logo-image no-lazy" src="<?php echo esc_url($logo_src); ?>" alt="<?php echo get_bloginfo('name'); ?>"
            width="64" height="64" <?php if (!empty($logo_src) && !empty($alt_logo_src)):  // Add data attributes only if BOTH logos exist. ?> data-logo-orig="<?php echo esc_url($logo_src); ?>"
                data-logo-alt="<?php echo esc_url($alt_logo_src); ?>" <?php endif; ?>>
